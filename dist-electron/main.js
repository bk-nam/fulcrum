...
import require$$1$3, { promisify as promisify$1 } from "util";
...
function requireJsonSchemaTraverse$1() {
  ...
  var traverse = jsonSchemaTraverse$1.exports = function(schema2, opts, cb) {
...
function requireCore$3() {
  if (hasRequiredCore$3) return core$4;
  hasRequiredCore$3 = 1;
  (function(exports$1) {
    Object.defineProperty(exports$1, "__esModule", { value: true });
...
const ajvFormatsModule = /* @__PURE__ */ getDefaultExportFromCjs(distExports$3);
...
class Conf {
  ...
  set(key, value) {
    ...
    const set2 = (key2, value2) => {
      ...
      if (this.#options.accessPropertiesByDotNotation) {
        setProperty$1(store2, key2, value2);
      } else {
  ...
  clear() {
    ...
    for (const key of Object.keys(this.#defaultValues)) {
      if (isExist(this.#defaultValues[key])) {
        ...
        if (this.#options.accessPropertiesByDotNotation) {
          setProperty$1(newStore, key, this.#defaultValues[key]);
        } else {
  ...
  */
  ...
  _set(key, value) {
    const { store: store2 } = this;
    setProperty$1(store2, key, value);
    this.store = store2;
  }
...
const WBS_TEMPLATE = `project_info:
  name: "Project Name"
...
function requireFind_process() {
  ...
  function fetchBin(cmd) {
    ...
    const last2 = pieces[pieces.length - 1];
    if (last2) {
      pieces[pieces.length - 1] = last2.split(" ")[0];
    }
    const fixed = [];
...
const findProcess = /* @__PURE__ */ getDefaultExportFromCjs(distExports$2);
...
function requireSrc() {
  ...
  (function(exports$1) {
    ...
    const log = debug_12.default("@kwsites/file-exists");
    ...
    function exists(path2, type2 = exports$1.READABLE) {
      return check(path2, (type2 & exports$1.FILE) > 0, (type2 & exports$1.FOLDER) > 0);
    }
...
function pick(source, properties2) {
  const out = {};
  properties2.forEach((key) => {
...
var init_scheduler = __esm({
  "src/lib/runners/scheduler.ts"() {
    ...
    Scheduler = class {
      ...
      next() {
        const { promise, id: id2 } = append(this.pending, createScheduledTask());
...
function completionDetectionPlugin({
  ...
  return {
    ...
    async action(_data, { spawned, close }) {
      ...
      spawned.on("close", (code2) => events.close(code2));
      spawned.on("exit", (code2) => events.exit(code2));
      try {
        await events.result;
        if (deferClose) {
          await delay(50);
...
async function isGitRepository(projectPath) {
  try {
    const gitPath = path$1.join(projectPath, ".git");
    await promises.access(gitPath);
    return true;
  } catch {
    return false;
  }
}
async function getCommitsSince(projectPath, sinceTimestamp) {
  try {
...
async function getRecentCommits(projectPath, hours = 24) {
  const sinceTimestamp = Date.now() - hours * 60 * 60 * 1e3;
...
async function getChangedFilesSince(projectPath, sinceTimestamp) {
  try {
    if (!await isGitRepository(projectPath)) {
      return [];
    }
...
const timeStore = new ElectronStore({
  name: "fulcrum-time-tracking"
});
function generateSessionId() {
  return `session-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
}
function getDateKey(timestamp2 = Date.now()) {
  const date = new Date(timestamp2);
  return date.toISOString().split("T")[0];
}
function getStartOfWeek(timestamp2 = Date.now()) {
  const date = new Date(timestamp2);
...
function startSession(projectPath, projectName) {
...
function updateProjectStats(session) {
  ...
  if (existing) {
    ...
    existing.weeklyTime = Object.entries(existing.dailyBreakdown).filter(([date]) => new Date(date).getTime() >= weekStart).reduce((sum, [, time]) => sum + time, 0);
...
function getProjectTimeStats(projectPath) {
  const projectStats = timeStore.get("projectStats", {});
...
function getTimeTrackingSummary() {
  ...
  const topProjects = allStats.sort((a, b) => b.totalTime - a.totalTime).slice(0, 5).map((stat) => ({
...
function recordActivity(projectPath, projectName, type2, description2, metadata2) {
  try {
...
async function enrichActivityWithGit(projectPath, activity) {
  try {
    if (!await isGitRepository(projectPath)) {
      return activity;
    }
    const recentCommits = await getRecentCommits(projectPath, 24);
    const lastActivityTime = activity.lastActivity?.timestamp || Date.now() - 24 * 60 * 60 * 1e3;
...
async function detectProjectType(projectPath) {
  try {
    const files = await promises.readdir(projectPath);
...
async function parseWbsMetadata(projectPath, lastModified) {
  try {
    ...
    if (Array.isArray(rawTechStack)) {
      techStack = rawTechStack.filter((item) => typeof item === "string");
      ...
      techStack = Object.values(rawTechStack).flat().filter((item) => typeof item === "string");
    }
    let currentPhase;
    if (wbsData.phases && Array.isArray(wbsData.phases)) {
      const activePhase = wbsData.phases.find((phase) => {
        if (!phase.tasks || !Array.isArray(phase.tasks) || phase.tasks.length === 0) {
          return false;
        }
        return phase.tasks.some((task) => task.status !== "Done");
      });
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        const type2 = await detectProjectType(projectPath);
...
async function createWbsTemplate(projectPath) {
  try {
    const wbsPath = path$1.join(projectPath, "wbs.yaml");
    await promises.writeFile(wbsPath, WBS_TEMPLATE, "utf-8");
  } catch (error) {
    console.error("Error creating WBS template:", error);
    throw error;
  }
}
async function readWbs(projectPath) {
  try {
    const wbsPath = path$1.join(projectPath, "wbs.yaml");
    const content = await promises.readFile(wbsPath, "utf-8");
    return content;
  } catch (error) {
...
async function saveWbs(projectPath, content) {
  try {
    const wbsPath = path$1.join(projectPath, "wbs.yaml");
    await promises.writeFile(wbsPath, content, "utf-8");
  } catch (error) {
...
function setupIpcHandlers() {
  ipcMain$1.handle("get-root-directory", async () => {
    try {
  ...
  ipcMain$1.handle("get-projects", async () => {
    try {
      const rootDirectory = store.get("rootDirectory");
      if (!rootDirectory) {
      ...
      try {
        await promises.access(rootDirectory);
      } catch {
  ...
  ipcMain$1.handle("get-settings", async () => {
    try {
      return getSettings();
    } catch (error) {
  ...
  ipcMain$1.handle("save-settings", async (_event, settings) => {
    try {
  ...
  ipcMain$1.handle("launch-project", async (_event, projectPath) => {
    try {
  ...
  ipcMain$1.handle("check-wbs-exists", async (_event, projectPath) => {
    try {
  ...
  ipcMain$1.handle("create-wbs-template", async (_event, projectPath) => {
    try {
  ...
  ipcMain$1.handle("read-wbs", async (_event, projectPath) => {
    try {
  ...
  ipcMain$1.handle("save-wbs", async (_event, projectPath, content) => {
    try {
  ...
  ipcMain$1.handle("save-quick-note", async (_event, note) => {
    try {
      ...
      if (note.projectPath) {
        const projectName = path$1.basename(note.projectPath);
  ...
  ipcMain$1.handle("save-virtual-project", async (_event, vp) => {
    try {
      const virtualProjects = store.get("virtualProjects", []);
      const now = Date.now();
      let savedProject;
  ...
  ipcMain$1.handle("delete-virtual-project", async (_event, vpId) => {
    try {
      const virtualProjects = store.get("virtualProjects", []);
  ...
  ipcMain$1.handle("materialize-virtual-project", async (_event, vpId, targetDirectory, folderName) => {
    try {
      const virtualProjects = store.get("virtualProjects", []);
      ...
      const projectPath = path$1.join(targetDirectory, folderName);
      ...
      const wbsContent = dump(wbsData, {
        indent: 2,
        lineWidth: 120,
        noRefs: true
      });
      const wbsPath = path$1.join(projectPath, "wbs.yaml");
      await promises.writeFile(wbsPath, wbsContent, "utf-8");
...
` : ""}
`;
      const readmePath = path$1.join(projectPath, "README.md");
      await promises.writeFile(readmePath, readmeContent, "utf-8");
  ...
  ipcMain$1.handle("get-current-focus-phase", async (_event, projectPath) => {
    try {
  ...
  ipcMain$1.handle("set-current-focus-phase", async (_event, projectPath, phaseName) => {
    try {
      const focusPhases = store.get("currentFocusPhases", {});
      if (phaseName === null) {
  ...
  ipcMain$1.handle("get-project-activity", async (_event, projectPath) => {
    try {
  ...
  ipcMain$1.handle("get-all-activities", async () => {
    try {
  ...
  ipcMain$1.handle("end-time-session", async (_event, projectPath) => {
    try {
  ...
  ipcMain$1.handle("get-project-time-stats", async (_event, projectPath) => {
    try {
  ...
  ipcMain$1.handle("get-time-tracking-summary", async () => {
    try {
